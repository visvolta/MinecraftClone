      ...
      unloadDistance_(std::max(0, renderDistance) + 1)
{
    ...
    const unsigned int generationWorkerCount =
        hardwareThreads >= 12U ? 3U : (hardwareThreads >= 6U ? 2U : 1U);
    generationThreads_.reserve(generationWorkerCount);
    for (unsigned int worker = 0; worker < generationWorkerCount; ++worker)
        generationThreads_.emplace_back(&World::generationWorker, this);

    const std::size_t initialChunkCapacity = static_cast<std::size_t>(
        (renderDistance_ * 2 + 3) * (renderDistance_ * 2 + 3));
    chunkManager_.reserve(initialChunkCapacity);
    desiredChunks_.reserve(initialChunkCapacity);
    generationInFlight_.reserve(initialChunkCapacity);
    meshInFlight_.reserve(initialChunkCapacity);
    meshVersions_.reserve(initialChunkCapacity);
    uploadedMeshVersions_.reserve(initialChunkCapacity);
    highPriorityMeshKeys_.reserve(initialChunkCapacity);
    initialMeshPending_.reserve(initialChunkCapacity);
    chunkMeshes_.reserve(initialChunkCapacity);
...
World::~World()
{
    ...
    for (std::thread& generationThread : generationThreads_)
    {
    ...
    for (int x = 0; x < Chunk::WIDTH; ++x)
    {
        for (int z = 0; z < Chunk::DEPTH; ++z)
        {
            for (int y = 0; y < Chunk::HEIGHT; ++y)
            {
                ...
                if (!hasChest || generatedBlock != BlockType::Chest)
                    continue;
                const BlockPosition position{
                    chunk.getWorldOriginX() + x,
                    y,
...
std::vector<ChunkSnapshot> World::getModifiedChunkSnapshots() const
{
    ...
    std::unordered_set<ChunkKey> loadedModified;
    loadedModified.reserve(modifiedChunks_.size());

    chunkManager_.forEachChunk([&](const Chunk* chunk)
    {
        if (chunk == nullptr) return;
        const ChunkKey key = makeKey(chunk->getChunkX(), chunk->getChunkZ());
        if (!modifiedChunks_.contains(key)) return;
        result.push_back(chunk->snapshot());
        loadedModified.insert(key);
    });

    for (const ChunkKey key : modifiedChunks_)
    {
        if (loadedModified.contains(key))
            continue;
...
int World::getHighestSolidBlockY(int worldX, int worldZ) const
{
    ...
    const int height = chunk->getMotionBlockingHeight(
    ...
    std::uint64_t version) const
{
    ...
    input.snapshot = std::make_unique_for_overwrite<ChunkMeshSnapshot>();
    ...
    for (int section = 0; section < Chunk::SECTION_COUNT; ++section)
    {
        ...
        for (int nz = 0; nz < 3; ++nz)
            ...
            {
                const Chunk* candidate = neighbourhood[nz][nx];
                if (candidate != nullptr && !candidate->isSectionEmpty(section))
                    highestNonEmptySection = std::max(
                        highestNonEmptySection, section);
            }
    }
    snapshot.copiedMaximumY = highestNonEmptySection < 0
        ? -1
        : (highestNonEmptySection + 1) * Chunk::SECTION_HEIGHT - 1;
    ...
    for (int y = 0; y <= snapshot.copiedMaximumY; ++y)
    {
        for (int z = -ChunkMeshSnapshot::Border;
             z < Chunk::DEPTH + ChunkMeshSnapshot::Border; ++z)
        {
