...
public:
    ...
    bool place(WorldGenerationContext&c,JavaRandom&r,const Box&clip) override
    {
        ...
        for(int section=0;section<sections;++section)
        {
            ...
            if(r.nextInt(100)==0){const int wx=worldX(2,z-1),wy=worldY(0),wz=worldZ(2,z-1);if(clip.contains(wx,wy,wz)&&isAir(c.getBlockState(wx,wy,wz))&&!isAir(c.getBlockState(wx,wy-1,wz))){setBlock(c,clip,state("rail",{{"shape",r.nextBoolean()?"north_south":"east_west"}}),2,0,z-1);(void)r.nextLong();}}
            if(r.nextInt(100)==0){const int wx=worldX(0,z+1),wy=worldY(0),wz=worldZ(0,z+1);if(clip.contains(wx,wy,wz)&&isAir(c.getBlockState(wx,wy,wz))&&!isAir(c.getBlockState(wx,wy-1,wz))){setBlock(c,clip,state("rail",{{"shape",r.nextBoolean()?"north_south":"east_west"}}),0,0,z+1);(void)r.nextLong();}}
            if(hasSpiders&&!spawnerPlaced){const int rz=z-1+r.nextInt(3);const int wx=worldX(1,rz),wy=worldY(0),wz=worldZ(1,rz);if(clip.contains(wx,wy,wz)&&skyBrightness(c,clip,1,0,rz)<8){spawnerPlaced=true;c.setBlockState(wx,wy,wz,tryState("mob_spawner").value_or(state("spawner")));}}
        }
